...
class EditProposalView(LoginRequiredMixin, View):
    ...
    def get(self, request, *args, **kwargs):
        return JsonResponse({"proposal": self.delegate.proposal})
    ...
    def put(self, request, *args, **kwargs):
        ...
        if form.is_valid():
            ...
            data.update({"updated": False, "errors": form.errors})
...
class EditContributionView(LoginRequiredMixin, View):
    ...
    def post(self, request, *args, **kwargs):
        ...
        if form.is_valid():
            ...
            data.update({"created": False, "errors": form.errors})
        return JsonResponse(data)
...
class EditNodeView(LoginRequiredMixin, View):
    ...
    def get(self, request, *args, **kwargs):
        node = get_object_or_404(Node, id=request.GET.get("id"), delegate_id=self.delegate.id)
    ...
    def post(self, request, *args, **kwargs):
        ...
        if form.is_valid():
            StatusUpdate.objects.create(
                delegate=self.delegate, message=form.cleaned_data["message"]
            )
            data.update({"created": True})
        else:
            data.update({"created": False, "errors": form.errors})

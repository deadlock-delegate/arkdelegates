    ...
    def get_context_data(self, delegate_slug, **kwargs):
        ...
        if self.request.user.is_authenticated and hasattr(self.request.user, "delegate"):
            ...
            can_edit_delegate = Delegate.objects.filter(
                user_id=self.request.user.id, slug=delegate_slug
            ).exists()
        else:
        ...
        context.update(
            {
                "seo": {
                    ...
                    "description": (
                        "Check what {} delegate has done for the Ark community, how many nodes "
                        "it runs and what's the proposal.".format(delegate_info["name"])
                    ),
                },

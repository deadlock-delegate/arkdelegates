...
def test_fetch_delegates_list_returns_correct_delegates_for_no_search_query():
    ...
    factories.HistoryFactory(delegate_fk=delegate_a, voters=456, rank=3, created=datetime.now())
...
@pytest.mark.parametrize("search_query", ["", "spongebob"])
@pytest.mark.django_db
def test_fetch_delegates_list_returns_delegate_with_correct_fields(search_query):
    ...
    factories.HistoryFactory(delegate_fk=delegate, voters=456, rank=3, created=datetime.now())
